...
angular.module('BE.seed.controller.mapping', []).controller('mapping_controller', [
  ...
  function (
    ...
    var profile_mappings_from_working_mappings = function () {
      ...
      return _.reduce(
        $scope.mappings,
        function (profile_mapping_data, mapping) {
          var this_mapping = {
    ...
    $scope.new_profile = function () {
      ...
      if (!$scope.mappingBuildingSync) {
        ...
        var currentProfileForBuildingSync =
          $scope.current_profile.profile_type !== undefined &&
          [COLUMN_MAPPING_PROFILE_TYPE_BUILDINGSYNC_DEFAULT, COLUMN_MAPPING_PROFILE_TYPE_BUILDINGSYNC_CUSTOM].includes(
            $scope.current_profile.profile_type
          );
    ...
    $scope.save_profile = function () {
      ...
      column_mappings_service
        .update_column_mapping_profile($scope.organization.id, profile_id, {
          mappings: profile_mapping_data
        })
    ...
    matching_criteria_columns_payload.PropertyState = _.map(
      matching_criteria_columns_payload.PropertyState,
      function (column_name) {
        var display_name = _.find($scope.mappable_property_columns, {
    ...
    matching_criteria_columns_payload.TaxLotState = _.map(
      matching_criteria_columns_payload.TaxLotState,
      function (column_name) {
        var display_name = _.find($scope.mappable_taxlot_columns, {
          column_name: column_name
        }).display_name;
        return {
          column_name: column_name,
    ...
    $scope.property_matching_criteria_columns = _.map(matching_criteria_columns_payload.PropertyState, 'display_name')
      .sort()
      .join(', ');
    $scope.taxlot_matching_criteria_columns = _.map(matching_criteria_columns_payload.TaxLotState, 'display_name')
      .sort()
      .join(', ');
    ...
    var eui_columns = _.filter($scope.mappable_property_columns, {
      data_type: 'eui'
    });
    $scope.is_eui_column = function (col) {
      ...
      return (
        ...
        Boolean(_.find(eui_columns, { column_name: col.suggestion_column_name }))
      );
    };
    ...
    $scope.is_area_column = function (col) {
      ...
      return (
        col.suggestion_table_name === 'PropertyState' &&
        Boolean(_.find(area_columns, { column_name: col.suggestion_column_name }))
      );
    };
    ...
    $scope.is_ghg_column = function (col) {
      return (
        col.suggestion_table_name == 'PropertyState' &&
        Boolean(_.find(ghg_columns, { column_name: col.suggestion_column_name }))
      );
    };
    ...
    $scope.updateInventoryTypeDropdown = function () {
      ...
      if (chosenTypes.length === 1)
        $scope.setAllFields = _.find($scope.setAllFieldsOptions, {
          value: chosenTypes[0]
        });
    ...
    $scope.updateColDuplicateStatus = function () {
      ...
      _.forEach($scope.mappings, function (col) {
        ...
        var dup_header =
          _.filter($scope.raw_columns, function (filter_col) {
        ...
        if (result === true) {
          organization_service.geocoding_columns(org_id).then(function (geocoding_columns) {
            $scope.property_geocoding_columns_array = _.map(geocoding_columns.PropertyState, function (column_name) {
              return _.find($scope.mappable_property_columns, {
                column_name: column_name
              }).display_name;
            });
            ...
            $scope.taxlot_geocoding_columns_array = _.map(geocoding_columns.TaxLotState, function (column_name) {
              return _.find($scope.mappable_taxlot_columns, {
                column_name: column_name
              }).display_name;
            });
    ...
    $scope.suggested_fields_present = function () {
      ...
      var intersections = _.intersectionWith(
        suggested_address_fields,
        $scope.mappings,
        function (required_field, raw_col) {
    ...
    $scope.required_property_fields_present = function () {
      var property_mappings_found = _.find($scope.mappings, {
        suggestion_table_name: 'PropertyState'
      });
    ...
    $scope.required_taxlot_fields_present = function () {
      var taxlot_mappings_found = _.find($scope.mappings, {
        suggestion_table_name: 'TaxLotState'
      });
    ...
    $scope.empty_units_present = function () {
      return Boolean(
        _.find($scope.mappings, function (field) {
          return (
    ...
    $scope.remap_buildings = function () {
      ...
      mapping_service.save_mappings($scope.import_file.id, $scope.get_mappings()).then(function () {
        // start re-mapping
    ...
    $scope.get_mapped_buildings = function () {
      ...
      $q.all([
        ...
        .finally(function () {
          ...
          data_quality_service
            .start_data_quality_checks_for_import_file($scope.organization.id, $scope.import_file.id)
            .then(function (response) {
              data_quality_service.data_quality_checks_status(response.progress_key).then(function (check_result) {
                ...
                $scope.data_quality_results = data_quality_service.get_data_quality_results(
                  $scope.organization.id,
                  check_result.unique_id
                );
    ...
    var display_cached_column_mappings = function () {
      ...
      _.forEach($scope.mappings, function (col) {
        ...
        var mappable_column = _.find($scope.mappable_property_columns.concat($scope.mappable_taxlot_columns), {
          column_name: cached_col.to_field,
          table_name: cached_col.to_table_name
        });
        if (mappable_column) {
