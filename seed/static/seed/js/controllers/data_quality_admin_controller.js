  ...
  function (
    ...
    $scope.data_types = [
      [
        { id: null, label: '' },
        {
          id: $scope.data_type_keys.string,
          label: $translate.instant('Text')
        }
      ],
      [
        { id: null, label: '' },
        {
          id: $scope.data_type_keys.number,
          label: $translate.instant('Number')
        },
      ...
      _.forEach($scope.ruleGroups, function (ruleGroups, inventory_type) {
        _.forEach(ruleGroups, function (ruleGroup) {
          var duplicate_rules = _.groupBy(ruleGroup, function (rule) {
            return `${
              rule.condition
            }-${rule.field}-${rule.data_type}-${rule.min}-${rule.max}-${rule.text_match}-${rule.units}-${rule.severity}-${!_.isUndefined(rule.label) ? rule.label : rule.status_label}`;
          });
    ...
    $scope.save_settings = function () {
      ...
      if ($scope.is_duplicate)
        return Notification.error({
          message: 'Duplicate rules detected.',
          delay: 10000
        });
    ...
    $scope.change_field = function (rule, oldField, index) {
      ...
      var newDataTypeString = _.find($scope.columns, {
        column_name: rule.field
      }).data_type;
    ...
    $scope.allEnabled = function () {
      var total = 0;
      var enabled = _.reduce(
        $scope.ruleGroups[$scope.inventory_type],
        function (result, ruleGroup) {
