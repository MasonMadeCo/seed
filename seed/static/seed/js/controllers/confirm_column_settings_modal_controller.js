  ...
  function (
    ...
    _.forEach($scope.change_summary_data, function (diff) {
      if (!_.isNil(diff.comstock_mapping)) {
        var found = _.find(all_columns, {
          related: true,
          comstock_mapping: diff.comstock_mapping
        });
        if (found) {
    ...
    if (
      _.find($scope.change_summary_data, {
    ...
    var format_preview_records = function (raw_inventory) {
      return _.reduce(
        raw_inventory,
        function (all_records, records, cycle_id) {
    ...
    var get_preview = function () {
      ...
      organization_service.match_merge_link_preview($scope.org_id, $scope.inventory_type, $scope.criteria_changes).then(function (response) {
        organization_service.check_match_merge_link_status(response.progress_key).then(function (completion_notice) {
          organization_service.get_match_merge_link_result($scope.org_id, completion_notice.unique_id).then(build_preview).then(preview_loading_complete);
        });
      });
    };
    ...
    if ($scope.matching_criteria_exists) {
      ...
      organization_service.matching_criteria_columns($scope.org_id).then(build_proposed_matching_columns).then(get_preview);
    }
