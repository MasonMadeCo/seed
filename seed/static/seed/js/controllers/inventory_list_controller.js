...
angular.module('BE.seed.controller.inventory_list', []).controller('inventory_list_controller', [
  ...
  function (
    ...
    $scope.display_labels = function (entity) {
      ...
      if ($scope.show_labels_by_inventory_id[id]) {
        for (let i in $scope.show_labels_by_inventory_id[id]) {
          ...
          labels.push(
            '<span class="',
            $scope.show_full_labels ? 'label' : 'label-bar',
            ' label-',
            label.label,
            '">',
            $scope.show_full_labels ? label.text : '',
            '</span>'
          );
      ...
      return [
        '<span title="',
        titles.join(', '),
        '" class="label-bars" style="overflow-x:scroll">',
        labels.join(''),
        '</span>'
      ].join('');
    };
    ...
    $scope.get_label_column_width = function () {
      ...
      let cells = renderContainer.querySelectorAll(
        '.' + uiGridConstants.COL_CLASS_PREFIX + col.uid + ' .ui-grid-cell-contents'
      );
    ...
    $scope.open_merge_modal = function (selectedViewIds) {
      ...
      var modalInstance = $uibModal.open({
        ...
        resolve: {
          ...
          data: function () {
            ...
            if ($scope.inventory_type === 'properties') {
              return inventory_service
                .get_properties(1, undefined, undefined, -1, selectedViewIds)
              ...
              return inventory_service
                .get_taxlots(1, undefined, undefined, -1, selectedViewIds)
                .then(function (inventory_data) {
                  _.forEach(selectedViewIds, function (id, index) {
                    var match = _.find(inventory_data.results, [viewIdProp, id]);
                    if (match) {
          ...
          has_meters: function () {
            if ($scope.inventory_type === 'properties') {
              return inventory_service.properties_meters_exist(selectedViewIds).then(function (has_meters) {
                return has_meters;
              });
            } else {
              return false;
    ...
    var taxlotFootprintColumn = _.find($scope.columns, {
    ...
    $scope.polygon = function (record, tableName) {
      ...
      if (!_.has(cache, record.id)) {
        ...
        var svg =
          ...
          width +
          '"><polygon points="' +
          _.initial(points).join(' ') +
          '" style="fill:#ffab66;stroke:#aaa;stroke-width:1;" /></svg>';
      ...
      data_quality_service.start_data_quality_checks(property_view_ids, taxlot_view_ids).then(function (response) {
        data_quality_service
          ...
          .then(function (result) {
            data_quality_service
              .get_data_quality_results($scope.organization.id, result.unique_id)
              .then(function (dq_result) {
                var modalInstance = $uibModal.open({
    ...
    _.map($scope.columns, function (col) {
      ...
      if (
        ...
        col.cellTemplate =
          '<div class="ui-grid-cell-contents" uib-tooltip="{{COL_FIELD CUSTOM_FILTERS}}" tooltip-append-to-body="true" tooltip-popup-delay="500">{{COL_FIELD CUSTOM_FILTERS}}</div>';
      }
      ...
      $scope.columns.unshift(
        {
          ...
          cellTemplate:
            ...
            '  <div title="' +
            $translate.instant('Merged Records') +
            '" class="ui-grid-cell-contents merged-indicator">' +
        ...
        },
        {
          ...
          cellTemplate:
            '<div class="ui-grid-row-header-link">' +
            '  <a title="' +
            $translate.instant('Go to Meters') +
        ...
        },
        {
          ...
          cellTemplate:
            '<div class="ui-grid-row-header-link">' +
            '  <a title="' +
            $translate.instant('Go to Detail Page') +
        ...
        },
        {
          ...
          displayName: '',
          headerCellTemplate:
            '<i ng-click="grid.appScope.toggle_labels()" class="ui-grid-cell-contents fa fa-chevron-circle-right" id="label-header-icon" style="margin:2px; float:right;"></i>',
          cellTemplate:
            '<div ng-click="grid.appScope.toggle_labels()" class="ui-grid-cell-contents" ng-bind-html="grid.appScope.display_labels(row.entity)"></div>',
          enableColumnMenu: false,
          enableColumnMoving: false,
          enableColumnResizing: false,
          enableFiltering: false,
    ...
    for (i in $scope.columns) {
      ...
      if (column['related']) {
        ...
        column['filterHeaderTemplate'] =
          '<div class="ui-grid-filter-container"><input type="text" title="' +
          title +
          '" class="ui-grid-filter-input" disabled=disabled />';
      }
    }
    ...
    var processData = function (data) {
      ...
      var visibleColumns = _.map($scope.columns, 'name').concat([
        '$$treeLevel',
        'notes_count',
      ...
      for (var i = 0, trueIndex = 0; i < roots; ++i, ++trueIndex) {
        ...
        for (var j = 0; j < related.length; ++j) {
          var updated = _.reduce(
            related[j],
            function (result, value, key) {
              if (_.includes(columnNamesToAggregate, key))
                aggregations[key] = (aggregations[key] || []).concat(_.split(value, '; '));
              result[key] = value;
              return result;
            },
            {}
        ...
        aggregations = _.pickBy(
          _.mapValues(aggregations, function (values, key) {
    ...
    var evaluateDerivedColumns = function () {
      ...
      const attached_derived_columns = derived_columns_payload.derived_columns.filter(col =>
        ...
        all_evaluation_results.push(
          ...batched_inventory_ids.map(ids => {
            return derived_columns_service
              .evaluate($scope.organization.id, col.id, $scope.cycle.selected_cycle.id, ids)
              .then(res => {
                formatted_results = res.results.map(x =>
                  typeof x.value == 'number'
                    ? {
                        ...x,
                        value: _.round(x.value, $scope.organization.display_decimal_places)
                      }
                    : x
                );
                return {
      ...
      return fetch(page, page_size).then(function (data) {
        if (data.status === 'error') {
          ...
          if (data.recommended_action === 'update_column_settings') {
            const columnSettingsUrl = $state.href('organization_column_settings', {
    ...
    $scope.run_action = function (viewIds = [], action = null) {
      ...
      if (viewIds.length > 0) {
        ...
        if ($scope.inventory_type === 'properties') {
          selectedViewIds = inventory_service
            .get_properties(
              undefined,
              undefined,
              $scope.cycle.selected_cycle,
        ...
        selectedViewIds = _.map(
          _.filter($scope.gridApi.selection.getSelectedRows(), {
            $$treeLevel: 0
          }),
          view_id_prop
        );
      ...
      switch (action) {
        ...
        case 'open_analyses_modal':
          ...
          break;
        case 'open_refresh_metadata_modal':
          $scope.open_refresh_metadata_modal(selectedViewIds);
          break;
        case 'open_geocode_modal':
          $scope.open_geocode_modal(selectedViewIds);
          break;
        case 'open_ubid_modal':
          $scope.open_ubid_modal(selectedViewIds);
          break;
          ...
          resolve: {
            ...
            inventory_payload: [
              ...
              function ($state, $stateParams, inventory_service) {
                return record.inventory_type === 'properties'
                  ? inventory_service.get_property(record.view_id)
                  : inventory_service.get_taxlot(record.view_id);
              }
            ],
            organization_payload: _.constant(organization_payload),
            notes: [
              'note_service',
              function (note_service) {
    ...
    const updateColumnFilterSort = function () {
      const columns = _.filter($scope.gridApi.saveState.save().columns, function (col) {
        return (
          _.keys(col.sort).filter(key => key !== 'ignoreSort').length +
            (_.get(col, 'filters[0].term', '') || '').length >
          0
        );
      });
    ...
    const restoreGridSettings = function () {
      ...
      if (!_.isNull(state)) {
        ...
        $scope.gridApi.saveState.restore($scope, state).then(function () {
    ...
    $scope.gridOptions = {
      ...
      onRegisterApi: function (gridApi) {
        ...
        gridApi.colMovable.on.columnPositionChanged($scope, function () {
          ...
          staticColIndex = _.findIndex($scope.gridApi.grid.columns, {
            name: 'merged_indicator'
          });
          if (staticColIndex !== 2) {
